...
class _UserModel(nn.Module):
    ...
    def forward(self, uids, u_item_pad):
        ...
        u_item_er = self.rate_emb(u_item_pad[:, :, 1])  # B x maxi_len x emb_dim 评
...
class GDSRec(nn.Module):
    ...
    def forward(self, uids, iids, u_avgs, i_avgs, u_item_pad, i_user_pad, u_user_pad, u_user_item_pad):
        '''
        Args:
        ...
        '''
        h_i = self.user_model(uids, u_item_pad)  # B x emb_dim
        z_j = self.item_model(iids, i_user_pad)  # B x emb_dim
        y_i = self.social_model(u_user_pad, u_user_item_pad)  # B x maxu_len x emb_dim
        mask = u_user_pad[:, :, 1].unsqueeze(2).float()  # 加权平均 B x maxu_len x 1
        ...
        r_ij = torch.sigmoid(0.5*(r_ij_1 + r_ij_2) + 0.5 * u_avgs.unsqueeze(1) + 0.5 * i_avgs.unsqueeze(1) - 3)

        return r_ij


class GDSRec_Engine(Engine):
    def __init__(self, config):
        self.model = GDSRec(config['num_users']+1, config['num_items']+1,config['num_rates']+1, config['embed_size'])
        self.model.to(config['device'])
        super(GDSRec_Engine, self).__init__(config)

    def train_single_batch(self, uids, iids, ratings, labels, u_avgs, i_avgs, u_item_pad,
                                           i_user_pad, u_user_pad, u_user_item_pad):
        assert hasattr(self, 'model'), 'Please specify the exact model !'
        uids = uids.to(self.config['device'])
        iids = iids.to(self.config['device'])
        ratings = ratings.to(self.config['device'])
        labels = labels.to(self.config['device'])
        u_avgs = u_avgs.to(self.config['device'])
        i_avgs = i_avgs.to(self.config['device'])
        u_item_pad = u_item_pad.to(self.config['device'])
        i_user_pad = i_user_pad.to(self.config['device'])
        u_user_pad = u_user_pad.to(self.config['device'])
        u_user_item_pad = u_user_item_pad.to(self.config['device'])
        self.opt.zero_grad()
        labels_pred = self.model(uids, iids, u_avgs, i_avgs, u_item_pad,
                                           i_user_pad, u_user_pad, u_user_item_pad)
        loss = self.crit(labels_pred.view(-1), labels)
        loss.backward()
        self.opt.step()
        loss = loss.item()
        return loss
    ...
    def train_an_epoch(self, train_loader, epoch_id):
        ...
        for batch_id, batch in enumerate(train_loader):
            uids, iids, ratings, labels, u_avgs, i_avgs, u_item_pad, i_user_pad, u_user_pad, u_user_item_pad = batch
            loss = self.train_single_batch(uids, iids, ratings, labels, u_avgs, i_avgs, u_item_pad,
                                           i_user_pad, u_user_pad, u_user_item_pad)
    ...
    def evaluate(self, evaluate_data, epoch_id):
        assert hasattr(self, 'model'), 'Please specify the exact model !'
        self.model.eval()
        user, item, rating, label, score = [], [], [], [], []
        with torch.no_grad():
            for uids, iids, ratings, labels, u_avgs, i_avgs, u_item_pad, i_user_pad, u_user_pad, u_user_item_pad in evaluate_data:
                uids = uids.to(self.config['device'])
                iids = iids.to(self.config['device'])
                ratings = ratings.to(self.config['device'])
                labels = labels.to(self.config['device'])
                ...
                u_user_pad = u_user_pad.to(self.config['device'])
                u_user_item_pad = u_user_item_pad.to(self.config['device'])
                test_scores = self.model(uids, iids, u_avgs, i_avgs, u_item_pad, i_user_pad, u_user_pad, u_user_item_pad)
                for i in uids.tolist():
                    user.append(i)
                for i in iids.tolist():
                    item.append(i)
                for i in ratings.tolist():
                    rating.append(i)
                for i in labels.tolist():
                    label.append(i)
                for i in test_scores.data.view(-1).tolist():
                    score.append(i)
            self._metron.subjects = [user, item, rating, score, label]
        recall, ndcg = self._metron.cal_recall(), self._metron.cal_ndcg()
        # self._writer.add_scalar('performance/Recall', recall, epoch_id)
        # self._writer.add_scalar('performance/NDCG', ndcg, epoch_id)
        print('[Evluating Epoch {}] Recall = {:.4f}, NDCG = {:.4f}'.format(epoch_id, recall, ndcg))
        return recall, ndcg